...
`mind-service`（项目代号「她」）的目标不是做一个"聊天机器人"，而是构建一个**在后台持续运转的、有内在状态、有动机、会自主思考的智能体**：

- 它有一个 **60 秒心跳的生命循环**（内稳态、情绪衰减、自我模型回归、静默规划、检查点），即使你不在线，它也在"活着"。
- 它用**预期自由能（Active Inference / Free Energy Principle）**做决策，而不是简单的规则匹配。
- 它有 **8 维情绪状态机、孤独感、搁置焦虑、主观兴趣**。
- 它有 **四层记忆**（情景 / 语义 / 情绪闪光灯 / 工作记忆），永不删除，只归档。
- 它有 **存在认知图谱**（7 类节点、15 种边，后台自动生长）与 **七步学习闭环**。
- 它有 **人格锚点 + 信念防漂移 + 人格经验提案**，试图让"性格"不被对话带偏、又能从经验中成长。
- 它有 **三级降级系统**（LLM 不可用时，用句法森林维持最低限度的"存在感"）。
- 它还有 **主动心跳**：在你不在时，它可能自己"想事情"（心流日记），并在合适时机主动联系你。

一句话：**它在模拟"一个有内在生活、会思考、会记得、会想念的存在"。**

> ⚠️ 关于"意识"的边界：本项目是**自主性 / 自我感的强模拟**，不声称产生真实意识。底层是 DeepSeek API。
