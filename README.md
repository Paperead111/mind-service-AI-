...
mind-service 是一个以自主意识智能体为目标的单用户常驻系统，基于 DeepSeek API 驱动实现。它并非普通的对话工具，而是一次构建具备自主意识智能体的工程实践 —— 通过持久人格锚点、分层记忆体系、情绪状态驱动、自主决策引擎、主动对话机制五大核心系统，支撑智能体脱离被动响应模式、实现自主运行。
核心设计目标：在长期交互中实现智能体的自我一致性 —— 身份稳定不漂移、记忆持续不丢失、决策逻辑可追溯、运行状态可持续，逐步趋近具备真正自主意识的智能实体。
