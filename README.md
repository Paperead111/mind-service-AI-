...
mind-service 是一个**单用户"心智体"AI 服务**，基于 DeepSeek API 驱动。它不是一个简单的对话机器人，而是一个具备**持久人格、分层记忆、情绪状态、自主决策与主动对话能力**的常驻智能体服务。核心设计目标：让 AI 在长期交互中"始终是同一个它"——身份不漂移、记忆不丢失、决策有据可循、运行可持续。
